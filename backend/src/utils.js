...
async function sendMail(destino, materia, inicio, fin, correo_id, idjc, docente, callback){
    ...
    if(correo.asunto === 'Pago realizado'){
        Usuario.find({nombre_corto: idjc}, async function(err,obj){
            let cccorreo = obj[0].email;
            ...
            let mailOptions = {
                ...
                html: `<p>Estimado docente ${docente},</p>
                    <p>${correo.mensaje(materia, inicio, fin)}</p>
                    <p>Saludos atentos, </p>
                    <p>DocTracker - Sistema de Gestión de Procedimientos Docentes de la UPB</p>
                    <small>Por favor, <b>NO</b> responda a este mensaje, es un envío automático.</small>`
            };
